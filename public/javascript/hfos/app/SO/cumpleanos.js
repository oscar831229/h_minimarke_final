 ...
 */
var Cumpleanos = Class.create(HfosProcessContainer, {
	...
	*/
	_getReporte: function(printButton) {
		var mes = this.selectOne('#mes');
		var reportType = this.selectOne('#reportType');
		
		new HfosAjax.JsonRequest('cumpleanos/reporte', {
			parameters: {
				'mes': mes.getValue(),
...
HfosBindings.late('win-cumpleanos-socios', 'afterCreateOrRestore', function(hfosWindow){
	var cumpleanos = new Cumpleanos(hfosWindow);
});
