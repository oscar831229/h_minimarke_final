 ...
 */
var TransaccionSocios = Class.create({
	...
	// Campos de Impuestos
	_totales: ['iva16r', 'iva16d', 'iva10r', 'iva10d', 'iva7r', 'iva7d', 'retencion', 'ica'],
	...
	_prepareForInput: function(){
		...
		cantidadElement.observe('change', this._actualizeValor.bind(this, cantidadElement));
	...
	_addReopenButton: function(eventName, response){
		if(this._type=='O'){
			...
			if(record.getValueFromName("estado")=="CERRADA"){
				this._hyperForm.addControlButton({
	...
	_getReferenciaOrReceta: function(){
		...
		this._form.disable();
		var almacenElement = this._form.selectOne('#almacen');
		Tatico.getReferenciaOrReceta(almacenElement.getValue(), itemElement.getValue(), function(response){
			this._form.enable();
	...
	_showData: function(data){
		...
		if(this._type=='C'||this._type=='T'||this._type=='A'){
	 ...
	 */
	_actualizeValor: function(cantidadElement){
		if(this._type=='E'||this._type=='C'){
			...
			if(cantidad==''){
				...
				if(cantidad>cantidadBase){
					if(this._type=='C'||this._type=='E'){
						if(this._wasLoaded==true){
							if(this._type=='C'){
								this._hyperForm.getMessages().error('La cantidad a entregar no puede superar la cantidad pedida');
							} else {
								this._hyperForm.getMessages().error('La cantidad recibida no puede superar la cantidad pedida');
							};
						...
						cantidadElement.setValue(cantidadBase);
					}
				}
			}
		} else {
	...
	_loadDetail: function(){
		...
		this._items.each(function(element){
			...
			this._hyperGrid.getField('cantidad').setValue(parseFloat(element.cantidad, 10) * this._cantidad);
			this._hyperGrid.getField('valor').setValue(parseFloat(element.costo, 10) * this._cantidad);
			var cantidadRecElement = this._hyperGrid.getField('cantidad_rec');
			if(cantidadRecElement!==null){
	...
	_updateTotales: function(element){
		...
		var impuestosDebito = ['iva16d', 'iva10d', 'iva7d'];
		...
		var impuestosCredito = ['iva16r', 'iva10r', 'iva7r'];
		...
		var otrosImpuestos = ['retencion', 'ica'];
	...
	_printTransaction: function(){
		...
		new HfosModalForm(this, 'tatico/getFormato', {
	...
	/**
	 * Intenta reabrir la orden de compra en pantalla
	 */
	...
	_calcularTransformacion: function(){
		...
		if(!cantidadObjetivo){
			...
			if(cantidad_objetivo.getValue()==''){
				vcantidadObjetivo = 0;
			} else {
				vcantidadObjetivo = cantidad_objetivo.getValue();
			}
		};
