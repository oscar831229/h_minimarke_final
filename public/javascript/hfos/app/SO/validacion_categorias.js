...
/**
 * Clase ValidacionCategorias
 *
 * Cada formulario de facturar en pantalla tiene asociado una instancia de esta clase
 */
var ValidacionCategorias = Class.create(HfosProcessContainer, {
	...
	*/
	_getReporte: function(printButton) {
		...
		new HfosAjax.JsonRequest('validacion_categorias/reporte', {
			parameters: {
				'reportType': reportType.getValue()
			},
			onCreate: function(){
				this.setIgnoreTermSignal(true);
				this.getMessages().notice('Se está realizando la validación, esto tardará algunos minutos...');
...
HfosBindings.late('win-validacion-categorias-socios', 'afterCreateOrRestore', function(hfosWindow){
	var validacionCategorias = new ValidacionCategorias(hfosWindow);
});
