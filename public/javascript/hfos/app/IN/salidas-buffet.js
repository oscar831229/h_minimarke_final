...
var SalidaBuffet = Class.create(HfosProcessContainer, {
	...
	_setBuscarFormCallbacks: function()
	{
		...
		if (buscarButton !== null) {
			buscarButton.observe('click', function(buscarButton){
				this.go('salidas_buffet/consultar', {
	...
	_saveFisico: function(saveButton)
	{
		...
		new HfosAjax.JsonRequest('salidas_buffet/guardar', {
...
HfosBindings.late('win-salidas-buffet', 'afterCreateOrRestore', function(hfosWindow) {
	new SalidaBuffet(hfosWindow);
});
