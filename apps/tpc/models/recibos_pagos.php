...
class RecibosPagos extends ActiveRecord {
	...
	/**
	 * Metodo para establecer el valor a _validar
	...
	/**
	 * Metodo para establecer el valor del campo calculos
	...
	/**
	 * Metodo para obtener el valor a _validar
	 * @return boolean
	 */
	...
	/**
	 * Devuelve el valor del campo calculos
	 * @return string
	 */
	...
	public function beforeValidationOnCreate(){
		...
		if($this->valor_pagado <= 0){
			$this->appendMessage(new ActiveRecordMessage('El valor pagado es obligatorio'));
			$status = false;
		}
		//Si tiene sociosId validar existencia
		if($this->socios_id>0){
			$socio = EntityManager::get('Socios')->findFirst($this->socios_id);
			if($socio==false){
				$this->appendMessage(new ActiveRecordMessage('Recibo Pago: El contrato dado no existe')); 
				return false;
			}
		}
		//Si tiene abonoReservasId validar existencia
		if($this->abono_reservas_id>0){
			$abonoReservas = EntityManager::get('AbonoReservas')->findFirst($this->abono_reservas_id);
			if($abonoReservas==false){
				$this->appendMessage(new ActiveRecordMessage('Recibo Pago: El abono a reserva dado no existe')); 
				return false;
			}
		}
		if($this->rc <= 0){
			$this->appendMessage(new ActiveRecordMessage('El numero de RC es obligatorio'));
			$status = false;
		}
		return $status;
	}
	...
	public function afterValidation(){
		...
		$socios->findFirst($this->socios_id);
		//Si esta anulado el contrato
		if($this->getValidar()==true){
			if($socios->getEstadoContrato()=='AA'){
				$this->appendMessage(new ActiveRecordMessage('Recibos Pagos: El contrato esta Anulado')); 
				return false;
			}
		}
		if($this->ciudad_pago<=0){
			$this->appendMessage(new ActiveRecordMessage('Recibo Pago: La ciudad es obligatoria')); 
			return false;
		}
		if($this->valor_pagado<=0){
			$this->appendMessage(new ActiveRecordMessage('Recibo Pago: El valor es obligatoria')); 
			return false;
		}
		
		//validamosque no haya un recibo de caja duplicado
		if($this->getValidar()==true){
			$recibosPagos = EntityManager::get('RecibosPagos');
			$recibosPagos->setConnection($this->getConnection());
			$existeRc = $recibosPagos->exists('rc='.$this->rc.' AND estado="V"');
			if($existeRc==true){
				$this->appendMessage(new ActiveRecordMessage('Recibo Pago: El consecutivo de Recibo de caja esta duplicado '.$this->rc)); 
				return false;
			}
		}
	}
	...
	public function initialize(){
		...
		$this->hasOne('cuentas_id','Cuentas','id');
	}
