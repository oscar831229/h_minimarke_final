...
class Socios extends RcsRecord {
	...
	/**
	 * Metodo que establece el estado de validar 
	 *
	 */
	public function setCambioContrato($cambioContato){
		$this->cambio_contrato = $cambioContato;
	}
	...
	/**
	 * Metodo que obtiene el valor de capital del cambio de contrato
	 *
	 */
	...
	/**
	 * Metodo que establece el valor de capital de cambio de contrato
	 *
	 */
	public function setValorCambioContrato($valorCambioContato){
		$this->valor_cambio_contrato = $valorCambioContato;
	}
	...
	public function beforeValidationOnCreate(){
		...
		//Obtenemos el fomrmato de numero d contrato por tipo de contrato
		if(TransactionManager::hasUserTransaction()==false){
			$this->appendMessage(new ActiveRecordMessage('Se requiere que la operación se realice bajo una transacción'));
			return false;
		}

		if(!$this->numero_contrato){
			$transaction = TransactionManager::getUserTransaction();
			$formatoContrato = TPC::getFormatoContrato($transaction, $this->tipo_contrato_id);
			$this->numero_contrato = $formatoContrato;
		}

		if($this->getValidar()==true){
			//verificamos si el contrato no existe		
			$exists = EntityManager::get('Socios')->count("numero_contrato='{$this->numero_contrato}'");
			if($exists>0){
				$this->appendMessage(new ActiveRecordMessage('Ya existe una contrato con ese número de contrato', 'numero_contrato'));
				return false;
			}
		}

		$this->estado_contrato = 'A'; //Activo
		$requestController = ControllerRequest::getInstance();
		if($requestController->isSetRequestParam('reservas_contrato')){
		   //Por ser creada desde activar contrato
		   $this->estado_movimiento = 'RC'; //Reserva con contrato
		} else {
		   //Por ser creada de forma normal
	...
	public function beforeValidation(){
		...
		$requestController = ControllerRequest::getInstance();
		$reservaNumContrato = $requestController->getParamPost('reservas_contrato');
		if(!empty($reservaNumContrato)){
		...
		if($this->getValidar() == true){
			...
			if($this->getEstadoContrato()=='AA'){
				$this->appendMessage(new ActiveRecordMessage('El contrato no puede modificarse '.$this->id, 'estado_contrato'));
				return false;
			}
		}
		if($this->id>0){
			if($this->getValidar()==true){
				//validamos que no edita si hay pagos ya hechos
				$recibosPagos = EntityManager::get('RecibosPagos');
				$recibosPagos->setConnection($this->getConnection());
				$reciboPago = $recibosPagos->findFirst(array('conditions' => 'socios_id='.$this->id));
				if($reciboPago != false){
					//throw new ActiveRecordException('aaa');
					$this->appendMessage(new ActiveRecordMessage('El contrato ya tiene pagos, no se puede modificar '.$this->id.', '.$this->_validar));
					return false;
				}
			}
		}
	...
	public function beforeCreate(){
		...
		//Actualizamos el consecutivo de tipo de contrato
		$tipoContrato = EntityManager::get('TipoContrato')->findFirst($this->tipo_contrato_id);
		$tipoContrato->setConnection($this->getConnection());
		$tipoContrato->setNumero($tipoContrato->getNumero()+1);
	...
	public function beforeDelete(){
		$this->appendMessage(new ActiveRecordMessage('Los contratos no pueden ser borrados', 'identificacion'));
		return false;
	}
	...
	public function initialize(){
		...
		$this->addForeignKey('estado_contrato', 'EstadoContrato', 'codigo', array(
			'message' => 'socios FK: El estado de contrato no es valido'
		));
		$this->addForeignKey('estado_movimiento', 'EstadoMovimiento', 'codigo', array(
		...
		$this->hasOne('estado_contrato', 'EstadoContrato', 'codigo');
		$this->hasOne('estado_movimiento', 'EstadoMovimiento', 'codigo');
	}
