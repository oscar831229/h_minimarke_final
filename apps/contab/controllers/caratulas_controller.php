 ...
 * CaratulasController
 *
 * Caratulas del Balance
 *
 */
class CaratulasController extends ApplicationController
{
	...
	public function generarAction(){
		...
		try
		{
			...
			$informe = array();
			$kontcs = $this->Kontc->find(array("informe='CAR' AND linea>0 AND tipo='N'", "order" => "linea"));
			...
			$titulo = new ReportText('CARATULAS DEL BALANCE', array(
				'fontSize' => 16,
				'fontWeight' => 'bold',
				'textAlign' => 'center'
			));
			...
			$report->setDocumentTitle('Caratulas del Balance');
			...
			$kontcs = $this->Kontc->find(array("informe='CAR' AND linea>0", "order" => "linea"));
			foreach($kontcs as $kontc){
				if($kontc->getTipo()=='L'){
			...
			$report->finish();
			$fileName = $report->outputToFile('public/temp/caratulas');
