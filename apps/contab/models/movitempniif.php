...
class Movitempniif extends ActiveRecord
{
	 ...
	 * @return string
	 */
	public function setCuentaMovi($cuentaMovi)
	{
		return $this->cuenta_movi = $cuentaMovi;
	}
	 ...
	 * @return string
	 */
	public function getNumeroMovi()
	{
		return $this->numero_movi;
	}
		...
		foreach ($this->getAttributes() as $attribute) {
			if ($attribute!='checksum' && $attribute != 'estado') {
				if ($attribute!='valor' && $attribute != 'base_grab') {
					...
					$checksum .= number_format((double) $this->$attribute, 2);
				}
			}
		}
		return md5($checksum);
