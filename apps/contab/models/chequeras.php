...
class Chequeras extends RcsRecord {
	...
	protected function validation(){
		...
		$minCheque = EntityManager::get('Cheque')->minimum(array('numero_cheque', 'conditions' => "chequeras_id='{$this->id}' AND estado='E'"));
		if($minCheque>0){
		...
		$maxCheque = EntityManager::get('Cheque')->maximum(array('numero_cheque', 'conditions' => "chequeras_id='{$this->id}' AND estado='E'"));
		if($maxCheque>0){
	...
	public function initialize(){
		...
		$this->hasMany('Cheque');
		$this->hasOne('FormatoCheque');
		$this->belongsTo('CuentasBancos');

		$this->addForeignKey('cuentas_bancos_id', 'CuentasBancos', 'id', array(
