...
class GruposDiferidos extends RcsRecord {
	...
	public function beforeSave(){
		...
		if($this->es_auxiliar=='N'){
			if($this->countDiferidos()>0){
				$this->appendMessage(new ActiveRecordMessage('No se puede establecer el grupo como no-auxiliar porque ya tiene activos diferidos asignados', 'es_auxiliar'));
				return false;
			}
		}
	...
	public function beforeDelete(){
		if($this->countDiferidos()){
			$this->appendMessage(new ActiveRecordMessage('No se puede eliminar el grupo porque tiene activos diferidos asociados', 'codigo'));
			return false;
		}
	}

	public function initialize(){
		$this->hasMany('grupo', 'Diferidos', 'linea');
	}
