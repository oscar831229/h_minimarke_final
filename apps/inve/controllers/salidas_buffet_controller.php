 ...
 *
 */
class Salidas_BuffetController extends ApplicationController
{
	...
	public function consultarAction()
	{
		...
		$saldosReferencias = new ActiveRecordJoin(array(
			...
			'conditions' => "{#Inve}.linea BETWEEN '$codigoLineaInicial' AND '$codigoLineaFinal' AND {#Saldos}.almacen='$codigoAlmacen'" .
				" AND ano_mes='0' AND {#Inve}.item = {#Saldos}.item AND {#Lineas}.linea = {#Inve}.linea AND {#Lineas}.almacen = 1 AND {#Inve}.rodizio = 'S'",
	...
	public function guardarAction()
	{
		...
		try {
			...
			for ($i = 0; $i < $numberItems; $i++) {
				...
				if (!isset($item[$i])) {
					return array(
						'status'  => 'FAILED',
						'message' => 'No existe la referencia en la línea '. ($i + 1)
					);
				}
			...
			'Comprobante' => $comprob,
				...
				'Observaciones' => 'SALIDA POR BUFFET ' . Date::getCurrentDate(),
				'Estado' => 'C',
				*/

			$comprob = sprintf('C%02s', $codigoAlmacen);
			...
			$movement = array(
				...
				'CentroCosto' => $almacen->getCentroCosto(),
				'NPedido' => 0,
				'FechaVencimiento' => $fechaMovimientoS,
				'Estado' => 'C',
				'VTotal' => 0,
				'Observaciones' => 'SALIDA POR BUFFET ' . Date::getCurrentDate(),
			);
		...
		return array(
			...
			'message' => 'Se realizó la salida por buffet con número:' . $numeros['inve'] . " y comprobante contable: " . $numeros['contab']
		);
	}
