...
class Almacenes extends RcsRecord {
			...
			foreach($tipoComprobs as $tipoComprob => $nombre){
				...
				if($comprob->save()==false){
					foreach($comprob->getMessages() as $message){
						$this->appendMessage(new ActiveRecordMessage('Comprobante '.$codigoComprob.': '.$message->getMessage(), $message->getField()));
					}
					return false;
				}
			}
	...
	protected function validation(){
		...
		$this->validate('InclusionIn', array(
			'field' => 'clase_almacen',
			'domain' => array('N', 'D'),
			'message' => 'La "Clase de Almacén" debe ser "PRINCIPAL" ó "DEPENDENCIA"',
			'required' => true
		));
...
/*
240810002001 / COMPRAS GRAVADAS 16%
Cuenta IVA 10% Descontable	240810002003 / COMPRAS GRAVADAS 10%
Cuenta IVA 16% Retenido	240810002007 / SERVICIOS GRABADOS 1.6%
Cuenta IVA 10% Retenido	236710005 / RETENIDO */