...
class Comprob extends ActiveRecord {
	...
	public function beforeSave(){
		if($this->consecutivo>0){
			...
			if($exists==true){
				...
				$siguiente = $Movi->maximum('numero', 'conditions: comprob="'.$this->codigo.'"')+1;
				if($siguiente>0){
		...
		if($this->cta_iva16_venta!=''){
			$exists = EntityManager::getEntityInstance('Cuentas')->count("cuenta='{$this->cta_iva16_venta}' AND es_auxiliar='S'");
			if($exists==false){
				$this->appendMessage(new ActiveRecordMessage('La cuenta de contabilización del iva 16% en la venta no existe ó no es auxiliar', 'cta_iva16_venta'));
				return false;
			}
		}
		if($this->cta_iva10_venta!=''){
			$exists = EntityManager::getEntityInstance('Cuentas')->count("cuenta='{$this->cta_iva10_venta}' AND es_auxiliar='S'");
			if($exists==false){
				$this->appendMessage(new ActiveRecordMessage('La cuenta de contabilización del iva 10% en la venta no existe ó no es auxiliar', 'cta_iva10_venta'));
				return false;
			}
		}
		if($this->comprob_contab!=''){
			if($this->comprob_contab==$this->codigo){
				$this->appendMessage(new ActiveRecordMessage('La comprobante contable no puede ser el mismo comprobante', 'comprob_contab'));
				return false;
			}
		}
	}
	...
	public function initialize(){
		...
		//Relaciones
		$this->hasMany('comprob', 'Movi', 'codigo');
		$this->hasMany('comprob', 'Movihead', 'codigo');
