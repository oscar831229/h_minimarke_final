 ...
 * Router
 *
 * Class for Front-Controller rewrite routing
 *
 * @category 	Kumbia
 * @package 	Router
 * @copyright	Copyright (c) 2008-2011 Louder Technology COL. (http://www.loudertechnology.com)
...
abstract class Router
{
	...
	/**
	 * Set additional parameters for route
	 *
	...
	/**
	 * Indicates whether the router is initialized or not
	 *
	 * @access public
	 * @static
	 */
	...
	/**
	 * Validates if router has a valid running state
	 *
	 * @access private
	 * @static
	 */
	static private function validateRouter(){
		if(self::$_initialized==false){
			throw new RouterException("Can't do routing in a invalid running state");
		}
	}

	/**
	 * Performs routing using a Uniform Resource Identifier
	 *
	 * @access public
	 * @static
	 * @param string $uri
	...
	/**
	 * Routes to current controller, another controller or another action
	 * Ex:
	 * <code>
	 * Router::routeTo("controller: name", ["action: name"], ["id: id"])
	 * </code>
	 *
	 * @access public
	 * @static
	...
	static public function routeTo($route){
		...
		if($cyclicRouting){
			...
			if(self::$_routedCyclic>=1000){
				throw new RouterException('A cyclic routing was detected. This may cause stability problems', 1000);
			}
		} else {
	...
	/**
	 * Returns the last URL routed by the Router
	 *
	 * @access public
	 * @static
	 */
	...
	/**
	 * Gets the URL within the current application
	 *
	 * @access public
	 * @static
	 */
	...
	/**
	 * Redirects the execution flow to other application in the same instance of the framework
	 *
	 * @access 	public
	 * @param 	string $uri
	 * @static
	...
	/**
	 * Indicates whether the invocation of an action was caused by a internal routing
	 *
	 * @access 	public
	 * @static
	 */
	...
	/**
	 * Detect hows should be treated router's parameters
	 *
	 * @access public
	 * @static
	 */
		...
		if (self::_detectRoutingType() == self::ROUTING_OTHER) {
			if (class_exists(self::$_routingAdapterType.'Router', false)==false) {
				if (Core::fileExists('Library/Kumbia/Router/Adapters/' . self::$_routingAdapterType . '.php')==false) {
					throw new RouterException("Routing adapter " . self::$_routingAdapterType . " doesn't exists");
				}
			}
		}
		/**
	...
	/**
	 * Detects the type of routing requested by the software client
	 *
	 * @return int
	 */
	static private function _detectRoutingType(){

		if(isset($_SERVER['HTTP_FLUXACTION'])){
			self::$_routingAdapterType = 'Flux';
		...
		if(isset($_SERVER['HTTP_X_JSON_ACCEPT'])){
			self::$_routingAdapterType = 'Json';
			return self::ROUTING_OTHER;
		}
	...
	/**
	 * Returns the routing adapter type used to comply the request
	 *
	 * @access 	public
	 * @return 	object
	 * @static
	 */
	static public function getRoutingAdapterType()
	{
		return self::$_routingAdapterType;
	}

	/**
	 * Returns the routing adapter used to comply the request
	 *
	 * @access 	public
	 * @return 	object
	 * @static
	...
	/**
	 * Reset the Router to serve a second request
	 *
	 * @access 	public
	 * @static
	 */
