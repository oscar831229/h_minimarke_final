...
abstract class CoreConfig
{
	...
	static public function readEnviroment()
	{
		...
		/*if (function_exists('apc_fetch')) {
			$cacheKey = 'env-' . Core::getInstanceName() . '-' . Router::getApplication();
		...
		//if (function_exists('apc_fetch')) {
		//	apc_store($cacheKey, $config, 1200);
		//}
	...
	{
		if ($application=='') {
			throw new CoreConfigException('Debe indicar el nombre de la aplicación donde está el archivo "'.$file.'" en '.$application);
		}
		if ($adapter=='') {
