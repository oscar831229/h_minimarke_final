...
abstract class Browse
{
	...
	static public function formsBrowse($form){
		...
		StandardGenerator::formsPrint("&nbsp;</center><div id='stdBrowse' align='center'>
		...
		if($db = DbPool::getConnection()){
			...
			StandardGenerator::formsPrint("</thead>");
			...
			if($db->numRows($q)){
				...
				while($row = $db->fetchArray($q)){
					StandardGenerator::formsPrint("</tr>\r\n<tr id='nTr$nTr' class='$color'>");
					foreach ($form['components'] as $name => $component) {
						if(!isset($component['notBrowse'])){
							$component['notBrowse'] = false;
						}
						...
						if (($component['type']!='hidden') && (!$component['notBrowse'])) {
							...
							if ($component['type']!='image') {
								StandardGenerator::formsPrint("<td align='center' style='border-left:1px solid #D1D1D1'
					...
					if (!$form['unableUpdate']) {
						StandardGenerator::formsPrint("<td style='border-left:1px solid #D1D1D1'><img src='".Core::getInstancePath()."img/edit.gif' title='Editar este Registro' style='cursor:pointer' onclick='window.location=\"".Core::getInstancePath().$app_controller."/query/&amp;$pk\"' alt=''/></td>");
					}
					if (!$form['unableDelete']) {
						StandardGenerator::formsPrint("<td style='border-left:1px solid #D1D1D1'><img src='".Core::getInstancePath()."img/delete.gif' title='Borrar este Registro' style='cursor:pointer' onclick='if(confirm(\"Seguro desea borrar este Registro?\")) window.location=\"".Core::getInstancePath().$app_controller."/delete/&amp;$pk\"' alt=''/></td>");
					}
					if ($color == $color2) {
						$color = $color1;
					} else {
				...
				StandardGenerator::formsPrint("</tr><tr><td bgcolor='#D0D0D0'
			...
			<table>
			 <tr>
			  ...
			  src='".Core::getInstancePath()."img/last.gif' alt=''/></a></td>
				...
				StandardGenerator::formsPrint("</table>");
				StandardGenerator::formsPrint("<div align='center' id='noFound'>No Hay Registros Para Visualizar</div>");
			}
			StandardGenerator::formsPrint("</form>");
