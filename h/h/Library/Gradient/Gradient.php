...
*        - fixed : not storeing proper gradient colors with image dimension greater than 255 (because of a limitation in imagecolorallocate)
...
class Gradient {
	...
	function __construct($w, $h, $d, $s, $e, $step=0, $file){
		$this->width = $w;
		$this->height = $h;
		$this->direction = $d;
		$this->startcolor = $s;
	...
	// stores the image with a portable function that works with any file type
	// depending on your server software configuration
	function store($im, $file){
	...
	static function setOpacity(&$img, $opacity){
		if(!isset($opacity)){
			return false;
		}
		$opacity /= 100;
