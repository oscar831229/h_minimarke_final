...
class Date extends Object
{
	...
	/**
	 * Devuelve el numero de meses de diferencia de un periodo inicial a uno final
	 * 
	 * @param int $periodoIni
	 * @param int $periodoFin
	 * 
	 * @return int $diff 
	*/
	static public function diffMonthsPeriod($periodoIni, $periodoFin)
	{
		$yearIni = substr($periodoIni,0,4);
		$monthIni = substr($periodoIni,4,2);

		$yearFin = substr($periodoFin,0,4);
		$monthFin = substr($periodoFin,4,2);

		$date1 = mktime(0,0,0,$monthIni,0,$yearIni); // m d y, use 0 for day
		$date2 = mktime(0,0,0,$monthFin,0,$yearFin); // m d y, use 0 for day
