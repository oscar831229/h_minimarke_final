...
abstract class StandardForm extends Controller
{
	...
	/**
	 * Numero de Minutos que el layout será cacheado
	 *
	 * @var integer
	 */
	...
	public function reportAction(){
		...
		StandardGenerator::scaffold($this->form, $this->scaffold);
	...
	/**
	 * Obtiene un modelo asociado al formulario cargado con los datos de entrada
	 *
	 * @return unknown
	 */
	public function _getPopulatedModel($updating){
		...
		$model = EntityManager::get($modelName);
		if(!$model->isDumped()){
			$model->dumpModel();
		}

		$numericTypes = $model->getDataTypesNumeric();
		if($updating){
			$primaryKey = array();
			foreach($this->form['components'] as $fieldName => $definition){
				if(isset($definition['primary'])&&$definition['primary']==1){
					if(isset($_REQUEST["fl_$fieldName"])){
						if(isset($numericTypes[$fieldName])){
							$value = $this->filter($_REQUEST["fl_$fieldName"], 'double');
						} else {
							$value = $this->filter($_REQUEST["fl_$fieldName"], 'striptags', 'extraspaces');
						}
			...
			if(count($primaryKey)){
				$model = $this->$modelName->findFirst(join(' AND ', $primaryKey));
			}
			$attributes = $model->getNonPrimaryKeyAttributes();
		} else {
			$attributes = $model->getAttributesNames();
		}


		Debug::add($numericTypes);
		...
		foreach($attributes as $fieldName){
			if(isset($_REQUEST["fl_$fieldName"])){
				...
				$model->writeAttribute($fieldName, $value);
			}
		}

		foreach ($this->form['components'] as $fieldName => $definition){
			if ($this->form['components'][$fieldName]['type']=='image'){
				if (isset($_FILES['fl_'.$fieldName])){
					if (!preg_match('/\.php$/', $_FILES['fl_'.$fieldName]['name'])){
						move_uploaded_file($_FILES['fl_'.$fieldName]['tmp_name'], htmlspecialchars('public/img/upload/'.$_FILES['fl_'.$fieldName]['name']));
						$model->writeAttribute($fieldName, urlencode(htmlspecialchars('upload/'.$_FILES['fl_'.$fieldName]['name'])));
					}
				}
			}
		}
	...
	public function insertAction()
	{
		...
		if ($controllerRequest->isPost()) {
			...
			//Obtener un modelo con la entrada
			$model = $this->_getPopulatedModel(false);
			...
			if(method_exists($this, 'beforeInsert')){
				if($this->beforeInsert($model)===false){
					$this->keep_action = 'insert';
					if(!Router::getRouted()){
						return $this->routeTo(array('action' => 'index'));
					}
				...
				if(isset($this->beforeInsert)){
					if($this->{$this->beforeInsert}($model)===false){
						$this->keep_action = 'insert';
						if(!Router::getRouted()){
							return $this->routeTo(array('action' => 'index'));
						}
			 ...
			 * por lo tanto los autonumericos no deben estar cargados
			 */
			$attributes = $model->getPrimaryKeyAttributes();
			...
			foreach($model->getAttributesNames() as $fieldName){
				if(isset($_REQUEST['fl_'.$fieldName])){
					$_REQUEST['fl_'.$fieldName] = $model->readAttribute($fieldName);
				}
			}
	...
	public function updateAction()
	{
		...
		$model = $this->_getPopulatedModel(true);
		 ...
		 */
		if(method_exists($this, 'validation')){
			if($this->validation($model)===false){
			...
			if(isset($this->validation)){
				if($this->{$this->validation}($model)===false){
					$this->keep_action = 'update';
					if(!Router::getRouted()){
						return $this->routeTo(array('action' => 'index'));
					}
		...
		if(method_exists($this, 'beforeUdate')){
			if($this->beforeUpdate($model)===false){
				$this->keep_action = 'update';
				if(!Router::getRouted()){
					return $this->routeTo(array('action' => 'index'));
				}
			...
			if(isset($this->beforeUpdate)){
				if($this->{$this->beforeUpdate}($model)===false){
					$this->keep_action = 'update';
					if(!Router::getRouted()){
						return $this->routeTo(array('action' => 'index'));
					}
		...
		if($model->update()==true){
			...
			$_REQUEST['id'] = $model->readAttribute('id');
			return $this->routeTo(array('action' => 'index'));
		}
	...
	public function indexAction()
	{
		...
		if ($this->scaffold == true) {
			...
			StandardGenerator::buildForm($this->form, true);
		} else {
			if (count($this->form)) {
				$this->form["source"] = $this->getSource();
				foreach ($this->ignoreList as $ignore) {
				...
				StandardGenerator::buildForm($this->form);
			} else {
