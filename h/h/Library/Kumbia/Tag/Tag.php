				...
				foreach($params[1] as $o){
					if($callback==false){
						if($value==$o->readAttribute($using[0])){
							if (trim($o->readAttribute($using[0])) == trim($o->readAttribute($using[1]))) {
									$code.="\t<option selected='selected' value='".trim($o->readAttribute($using[0]))."'>".trim($o->readAttribute($using[1]))."</option>\r\n";
							} else {
								$code.="\t<option selected='selected' value='".trim($o->readAttribute($using[0]))."'>".trim($o->readAttribute($using[0]))." - ".trim($o->readAttribute($using[1]))."</option>\r\n";
							}
						} else {
							if (trim($o->readAttribute($using[0])) == trim($o->readAttribute($using[1]))) {
								$code.="\t<option value='".trim($o->readAttribute($using[0]))."'>".trim($o->readAttribute($using[1]))."</option>\r\n";
							} else {
								$code.="\t<option value='".trim($o->readAttribute($using[0]))."'>".trim($o->readAttribute($using[0]))." - ".trim($o->readAttribute($using[1]))."</option>\r\n";
							}
						}
					} else {
