...
class SocioTcHyperComponent extends UserComponent {

	private static function _getNombreSocio($value){
		if($value!==null&&$value!==''){
			$socio = self::getModel('Socios')->findFirst($value);
			if($socio!=false){
				return utf8_encode($socio->getNumeroContrato()." - ".$socio->getNombres()." ".$socio->getApellidos());
			} else {
	...
	public static function build($name, $component, $value=null, $context=null){
		...
		<script type="text/javascript">HfosCommon.addSocioTcCompleter("'.$name.'", "'.$context.'")</script>';
		return $code;
	}
