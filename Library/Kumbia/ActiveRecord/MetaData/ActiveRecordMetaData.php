...
abstract class ActiveRecordMetaData
{
	...
	/**
	 * Guarda los meta-datos solo si fueron cambiados
	 *
	 * @var boolean
	 */
	static private $_changed = false;
	...
	static private function _getAPCKey($source)
	{
		if(!isset(self::$_apcKeys[$source])){
			...
			$sharedKey = $instance . Router::getApplication() . $source;
	...
	static public function dumpMetaData($table, $schema, $metaData){
		...
		#endif
		$tableMetadata = array();
		$tableMetadata[self::MODELS_ATTRIBUTES] = $fields;
		$tableMetadata[self::MODELS_PRIMARY_KEY] = $primaryKey;
		$tableMetadata[self::MODELS_NON_PRIMARY_KEY] = $nonPrimary;
		$tableMetadata[self::MODELS_NOT_NULL] = $notNull;
		$tableMetadata[self::MODELS_DATA_TYPE] = $dataType;
		$tableMetadata[self::MODELS_DATA_TYPE_NUMERIC] = $dataTypeNumeric;
		$tableMetadata[self::MODELS_DATE_AT] = $at;
		$tableMetadata[self::MODELS_DATE_IN] = $in;
		self::$_metaData[$schema][$table] = $tableMetadata;
		...
		/*if ($enviroment == 'production') {
			...
			if (self::$_hasAPC == true) {
				$sharedKey = self::_getAPCKey($source);
				apc_store($sharedKey, self::$_metaData[$schema][$table], 43200);
				unset($sharedKey);
			} else {
				self::_initializeSharedMemory();
				if(self::$_hasSharedMemory==true){
