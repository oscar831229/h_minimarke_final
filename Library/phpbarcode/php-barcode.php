...
$font_loc = 'Library/phpbarcode/arialbd.ttf';
...
function barcode_outimage($text, $bars, $scale = 1, $mode = "png", $total_y = 0, $space = ''){
	...
	$bar_color = array(0x00, 0x00, 0x00);
	$text_color = array(0x00, 0x00, 0x00);
	...
	while(list($n, $v)=each($chars)){
		if(trim($v)){
			...
			#Debug::add($font_loc);
			imagettftext($im, $fontsize, 0, $space['left']+($scale*$inf[0])+2, $fontheight, $col_text,
			'Library/phpbarcode/arialbd.ttf', $inf[2]);
		}
	}
